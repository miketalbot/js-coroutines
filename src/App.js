...
import {
  ...
  map,
  reduce,
} from "./component/array-utilities";
...
function App() {
  ...
  async function asyncFunctions() {
    ...
    add(
      `ASYNC: doubled the array 12 times to an array of ${format(
        o.length
      )} items`
    );
    let output = await stringifyAsync(o);
    add(
      `ASYNC: stringified array to a ${format(output.length)} character string`
    );
    let compressed = await compressAsync(output);
    add(
      `ASYNC: compressed the output to a string of ${format(
        compressed.length
      )} characters`
    );
    let decompressed = await decompressAsync(compressed);
    add(
      `ASYNC: decompressed the output to a string of ${format(
        decompressed.length
      )} characters - ${decompressed === output ? "Matches" : "Didn't match"}`
    );

    let justIds = await mapAsync(o, (v) => v.id);
    add(
      `ASYNC: mapped the ids from objects in the array to a new array of ${format(
        justIds.length
      )} items`
    );
    output = await stringifyAsync(justIds);
    add(
      `ASYNC: stringified just the ids in the array to a ${format(
        output.length
      )} character string`
    );
  }
