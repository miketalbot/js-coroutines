...
function needsSignature(doclet) {
    ...
    if (doclet.kind === 'function' || doclet.kind === 'class' || doclet.kind==='component') {
        needsSig = true
        ...
        if (symbols[module.longname]) {
            ...
            // we want to show the constructor-signature heading no matter what.
                .filter(function (symbol) {
                    return symbol.description || symbol.kind === 'class' || symbol.kind === 'component'
                })
                .map(function (symbol) {
                    symbol = doop(symbol)

                    if (symbol.kind === 'class' || symbol.kind === 'function' || symbol.kind === "component") {
...
function buildNav(members) {
    ...
    nav += buildMemberNav(members.globals.filter(g=>g.longname.startsWith("use")), 'Hooks', seen, linkto)
...
exports.publish = function (taffyData, opts, tutorials) {
    var conf, templatePath, indexUrl, globalUrl, sourceFiles, sourceFilePaths, staticFilePaths, staticFileFilter,
        staticFileScanner
    ...
    data().each(function (doclet) {
        ...
        if(doclet.description) {
            doclet.description = converter.makeHtml(doclet.description)
        }
    ...
    data().each(function (doclet) {
        var docletPath
        var component = doclet.kind === 'component'
        component && (doclet.kind = "class")
        var url = helper.createLink(doclet)
        component && (doclet.kind = "component")
    ...
    members.components = helper.find(data, {kind: "component"})
